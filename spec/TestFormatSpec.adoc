...
==== *Optional, Model-defined Macros*
  ...
  `RVMODEL_TARGET_INIT`::
    - contains initialization code for the test-target; may include code to copy the data sections
    from boot device to ram.  Or any other code that needs to be run prior to running the tests. 
  ...
  `RVMODEL_CHECK_TEST(ScrReg, Reg, Value)`:: 
    - Check that GPR should have value +
    - Outputs a debug message based on Reg!=Value or Reg=Value +
    - Used to help debug what tests have passed/failed
