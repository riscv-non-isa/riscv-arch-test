...
.macro RVTEST_INVISIBLE_TRAP_HANDLER_CODE
  ...
  invisible_Mcustom:
    // Give the model a chance to emulate the instruction first.
      ...
      // A handled instruction needs no register write.
      li      T5, 1
      bne     T4, T5, 1f
      j       invisible_Mtrap_return
      1:
        // Write the result to the instruction's destination register.
        li      T5, 2
        bne     T4, T5, 1f
        j       invisible_Mwrite_gpr
      1:
        li      T5, 0
        beq     T4, T5, invisible_Mtime
  ...
  invisible_Mtime:
    ...
    invisible_Mtime_read:
      ...
      #if UDB_MXLEN == 32
        srli    T4, T2, 27                        // timeh differs from time in CSR address bit 7
        andi    T4, T4, 1
        slli    T4, T4, 2
        add     T3, T3, T4
      #endif
  ...
  invisible_Mnot_handled:
    ...
    li      T5, CAUSE_ILLEGAL_INSTRUCTION     // the custom hook may use T5 as scratch
    #ifdef S_SUPPORTED
      // Illegal instructions were removed from medeleg. Forward an unhandled
      // lower-mode trap to S-mode as if hardware delegation had occurred.
      csrr    T1, mstatus
      LI(     T4, MSTATUS_MPP)
      and     T3, T1, T4
      ...
      // Apply the S-mode trap-entry state changes before returning to stvec.
      ...
      csrs    mstatus, T4
      csrr    T1, stvec
      andi    T1, T1, -4                        // exceptions use the stvec base in either mode
      csrw    mepc, T1
      // Return through the saved M-mode trap state after installing S-mode state.
  ...
  // Write the emulation result in T3 to the trapped instruction's rd.
