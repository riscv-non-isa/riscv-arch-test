...
covergroup Zicfilp_s_cg with function sample(ins_t ins);
    ...
    cp_zicfilp_indirect_elp_state_update: cross priv_mode_s, menvcfg_lpe, indirect_ct_prev, rs1_all_prev, lpad_dest;

    `ifdef COVER_ZCA
        cp_zicfilp_indirect_elp_state_update_c: cross priv_mode_s, menvcfg_lpe, indirect_ct_prev_c, rs1_all_prev_c, lpad_dest;
    `endif
    ...
    cp_zicfilp_lpad_label_exception_delivery: cross priv_mode_s, elp_before, sw_check_exc, xtval_lpad {
        ignore_bins ig_no_lp = binsof(elp_before.no_lp_expected);
    }

    cp_disabled_zicfilp: cross priv_mode_s, lpe_disabled, lpad_lpl_nonzero;
...
function void zicfilp_s_sample(int hart, int issue, ins_t ins);
    Zicfilp_s_cg.sample(ins);
endfunction
