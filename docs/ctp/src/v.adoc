...
Both upstream fixes have since landed (sail-riscv PR #1703 was merged in June 2026 and issue #1709 was closed in May 2026), but the skips have not yet been removed.  The `vstart` generation lines and the `vmv<nr>r.v` tests should be re-enabled once they have been verified against a Sail build that includes those fixes.
...
<<t-Zvknha-coverpoints>> and <<t-Zvknhb-coverpoints>> list the vector hashing instructions for Zvknha and Zvknhb respectively.

include::{testplansdir}/Zvknha.adoc[]
include::{testplansdir}/Zvknhb.adoc[]
