...
from testgen.data.ties import TIE_FRM_MODES, tie_edges_for
...
@add_coverpoint_generator("cp_fs1_edges_ties")
def make_fs1_edges_ties(instr_name: str, instr_type: str, coverpoint: str, test_data: TestData) -> list[TestChunk]:
    """Generate tests for fs1 exact-tie values under RNE and RMM."""
    ...
    for edge_val in edges:
        for frm_mode in TIE_FRM_MODES:
            ...
            bin_name = f"b{edge_val:#x}_{frm_mode}"
            desc = (
                f"{coverpoint} (Test source fs1 value = {test_data.flen_format_str.format(edge_val)}, frm = {frm_mode})"
            )
            tc = format_single_testcase(instr_name, instr_type, test_data, params, desc, bin_name, coverpoint)
            test_chunks.append(tc)
            return_testcase_registers(test_data, params)
