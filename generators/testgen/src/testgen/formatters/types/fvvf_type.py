##################################
# fvvf_type.py
#
# rwolk@hmc.edu August 2026
# SPDX-License-Identifier: Apache-2.0
##################################

from typing import Literal

from testgen.asm.helpers import load_float_reg
from testgen.asm.vector_helpers import (
    VectorLoad,
    handle_parameter_exclusions,
    handle_vector_fp,
    load_test_vtype,
    load_vec_regs,
    prep_mask_v,
    write_sigupd_v,
    write_sigupd_v_len,
)
from testgen.data.params import InstructionParams
from testgen.data.state import TestData
from testgen.formatters.registry import InstructionTypeConfig, VectorTypeConfig, add_instruction_formatter

fvvf_config = InstructionTypeConfig(
    instruction_class=["vector_fp"], required_params={"vd", "fs1", "vs2"}, vector_data=VectorTypeConfig()
)
fwvf_config = InstructionTypeConfig(
    instruction_class=["vector_fp"],
    required_params={"vd", "fs1", "vs2"},
    vector_data=VectorTypeConfig(overlap_constraints={("vd_bottom", "vs2")}, widened_regs={"vd"}),
)
fwwf_config = InstructionTypeConfig(
    instruction_class=["vector_fp"],
    required_params={"vd", "fs1", "vs2"},
    vector_data=VectorTypeConfig(widened_regs={"vd", "vs2"}),
)
fvvfm_config = InstructionTypeConfig(
    instruction_class=["vector_fp"],
    required_params={"vd", "fs1", "vs2", "maskval"},
    vector_data=VectorTypeConfig(overlap_constraints={("vd", "v0"), ("vs2", "v0")}),
)
fvvf_acc_config = InstructionTypeConfig(
    instruction_class=["vector_fp"], required_params={"vd", "fs1", "vs2"}, vector_data=VectorTypeConfig()
)
fwvf_acc_config = InstructionTypeConfig(
    instruction_class=["vector_fp"],
    required_params={"vd", "fs1", "vs2"},
    vector_data=VectorTypeConfig(overlap_constraints={("vd", "vs2")}, widened_regs={"vd"}),
)
fvvfp_config = InstructionTypeConfig(
    instruction_class=["vector_fp"],
    required_params={"vd", "fs1", "vs2"},
    vector_data=VectorTypeConfig(overlap_constraints={("vd", "vs2")}),
)
fvvfp_down_config = InstructionTypeConfig(
    instruction_class=["vector_fp"], required_params={"vd", "fs1", "vs2"}, vector_data=VectorTypeConfig()
)
fvvfm_config = InstructionTypeConfig(
    instruction_class=["vector_fp"],
    required_params={"vd", "fs1", "vs2", "maskval"},
    vector_data=VectorTypeConfig(overlap_constraints={("vd", "v0"), ("vs2", "v0")}),
)


@add_instruction_formatter("FVVF", fvvf_config)
def format_fvvf(
    instr_str: str, test_data: TestData, params: InstructionParams
) -> tuple[list[str], list[str], list[str]]:
    return format_fvvf_like_type(instr_str, test_data, params, "FVVF")


@add_instruction_formatter("FWVF", fwvf_config)
def format_fwvf(
    instr_str: str, test_data: TestData, params: InstructionParams
) -> tuple[list[str], list[str], list[str]]:
    return format_fvvf_like_type(instr_str, test_data, params, "FWVF", widen={"vd"})


@add_instruction_formatter("FWWF", fwwf_config)
def format_fwwf(
    instr_str: str, test_data: TestData, params: InstructionParams
) -> tuple[list[str], list[str], list[str]]:
    return format_fvvf_like_type(instr_str, test_data, params, "FWWF", widen={"vd", "vs2"})


@add_instruction_formatter("FVVFM", fvvfm_config)
def format_fvvfm(
    instr_str: str, test_data: TestData, params: InstructionParams
) -> tuple[list[str], list[str], list[str]]:
    assert params.maskval is not None, "Masks are required for FVVFM-Type Instructions"
    setup, test, check = format_fvvf_like_type(instr_str, test_data, params, "FVVFM")
    # Overwrite the test, as otherwise it generates with v0.t
    test = [f"{instr_str} v{params.vd}, v{params.vs2}, f{params.fs1}, v0"]
    return setup, test, check


@add_instruction_formatter("FVVF_ACC", fvvf_acc_config)
def format_fvvf_acc(
    instr_str: str, test_data: TestData, params: InstructionParams
) -> tuple[list[str], list[str], list[str]]:
    setup, test, check = format_fvvf_like_type(instr_str, test_data, params, "FVVF_ACC")
    # Overwrite the test, as otherwise it generates in the wrong order
    if params.maskval:
        test = [f"{instr_str} v{params.vd}, f{params.fs1}, v{params.vs2}, v0.t"]
    else:
        test = [f"{instr_str} v{params.vd}, f{params.fs1}, v{params.vs2}"]
    return setup, test, check


@add_instruction_formatter("FWVF_ACC", fwvf_acc_config)
def format_fwvf_acc(
    instr_str: str, test_data: TestData, params: InstructionParams
) -> tuple[list[str], list[str], list[str]]:
    setup, test, check = format_fvvf_like_type(instr_str, test_data, params, "FWVF_ACC", widen={"vd"})
    # Overwrite the test, as otherwise it generates in the wrong order
    if params.maskval:
        test = [f"{instr_str} v{params.vd}, f{params.fs1}, v{params.vs2}, v0.t"]
    else:
        test = [f"{instr_str} v{params.vd}, f{params.fs1}, v{params.vs2}"]
    return setup, test, check


@add_instruction_formatter("FVVFP", fvvfp_config)
def format_fvvfp(
    instr_str: str, test_data: TestData, params: InstructionParams
) -> tuple[list[str], list[str], list[str]]:
    return format_fvvf_like_type(instr_str, test_data, params, "FVVFP", enable_vs2_preload=True)


@add_instruction_formatter("FVVFP_DOWN", fvvfp_down_config)
def format_fvvfp_down(
    instr_str: str, test_data: TestData, params: InstructionParams
) -> tuple[list[str], list[str], list[str]]:
    return format_fvvf_like_type(instr_str, test_data, params, "FVVFP_DOWN", enable_vs2_preload=True)


def format_fvvf_like_type(
    instr_str: str,
    test_data: TestData,
    params: InstructionParams,
    type_name: str,
    *,
    widen: set[str] | None = None,
    enable_vs2_preload: bool = False,
) -> tuple[list[str], list[str], list[str]]:
    assert params.fs1 is not None and params.fs1val is not None, (
        f"fs1 and fs1val must be provided for {type_name}-type instructions"
    )
    assert params.vs2 is not None and params.vs2_val_pointer is not None, (
        f"vs2 and vs2_val_pointer must be provided for {type_name}-type instructions"
    )
    assert params.vd is not None and params.vd_val_pointer is not None, (
        f"vd and vd_val_pointer must be provided for {type_name}-type instructions"
    )
    assert params.temp_reg is not None, f"temp_reg must be provided for {type_name}-type instructions"
    assert params.sew is not None, f"sew must be provided for {type_name}-type instructions"
    assert params.sew in [16, 32, 64], "SEW must be 16, 32, or 64 for vector floating point instructions"
    assert params.lmul is not None, f"lmul must be provided for {type_name}-type instructions"
    assert test_data.test_chunk is not None, f"format_{type_name.lower()}_type must be used with an active TestChunk"
    assert params.csr_frm_val is not None, (
        "All Vector-FP instructions use the dyn rounding mode, so params.csr_frm_val MUST be provided"
    )

    if widen is None:
        widen = set()

    test_data.test_chunk.vector_labels.extend(
        [
            (params.vs2_val_pointer, *test_data.vector_labels[params.vs2_val_pointer]),
            (params.vd_val_pointer, *test_data.vector_labels[params.vd_val_pointer]),
        ]
    )

    setup = []

    # Setup Mask
    if params.maskval:
        setup.extend(prep_mask_v(params.maskval, test_data, params, clobber_vd=True))

    vd_vl = params.vl if params.vector_suite == "base" else "vlmax"
    vs2_vl = params.vl if params.vector_suite == "base" or not enable_vs2_preload else "vlmax"

    to_load = [
        VectorLoad(reg="vd", widen="vd" in widen, vl=vd_vl, no_fractional_load=True),
        VectorLoad(reg="vs2", widen="vs2" in widen, vl=vs2_vl),
    ]

    load_code, random_vl_reg = load_vec_regs(to_load, params, test_data)
    setup.extend(load_code)

    fp_load_sew_map: dict[int, Literal["half", "single", "double", "quad"]] = {16: "half", 32: "single", 64: "double"}
    setup.append(load_float_reg("fs1", params.fs1, params.fs1val, test_data, fp_load_sew_map[params.sew]))

    setup.append(load_test_vtype(params, random_vl_reg))

    # We don't need random_vl_reg anymore
    if random_vl_reg.startswith("x"):
        test_data.int_regs.return_register(int(random_vl_reg[1:]))

    if params.maskval:
        test = [f"{instr_str} v{params.vd}, v{params.vs2}, f{params.fs1}, v0.t"]
    else:
        test = [f"{instr_str} v{params.vd}, v{params.vs2}, f{params.fs1}"]

    # Return non-vd registers, so that we have enough for length-suite sigupd
    test_data.vec_regs.deallocate_operand("vs2")
    test_data.vec_regs.deallocate_operand("vs1")

    if params.vector_suite == "length":
        sig_lmul = params.lmul * (2 if "vd" in widen else 1)
        check = [*write_sigupd_v_len(test_data, params, sig_lmul, widen_vd="vd" in widen)]
    else:
        check = [*write_sigupd_v(test_data, params, widen_vd="vd" in widen)]

    # This can only be released after sigupd
    if params.maskval:
        test_data.vec_regs.return_register(0)

    handle_parameter_exclusions(params.lmul, setup, check)

    handle_vector_fp(setup, check, params.csr_frm_val, test_data)

    return (setup, test, check)
