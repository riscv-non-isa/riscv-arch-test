...
The ACTs use the RISC-V Sail model to generate expected results. It is currently compatible with version 0.14 of the model.

To install the sail model:

```bash
curl --location https://github.com/riscv/sail-riscv/releases/download/0.14/sail-riscv-$(uname)-$(arch).tar.gz | sudo tar xvz --directory=/path/to/install --strip-components=1
```
