...
`coverage`:: This directory contains the coverage reports of each test-suite organized using the test-pool structure guideline highlighted above. The reports are available in the html and yaml formatd. The directory also contains the `coverpoint.yaml` file which indicates all the coverpoints of interest which are covered by the tests in the `riscv-test-suite` directory.
...
1. Create a folder under the `riscv-target` diretory with the name of the target/device. In our case it would be : `riscv-target/spike`
